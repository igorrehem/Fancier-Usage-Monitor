...
const RELEASE_ASSET_NAME: &str = "fancier-usage-monitor.exe";
...
const WINGET_PACKAGE_ID: &str = "igorrehem.FancierUsageMonitor";
...
fn updates_dir() -> Result<PathBuf, String> {
    dirs::data_local_dir()
        .map(|dir| dir.join("FancierUsageMonitor").join("updates"))
        .or_else(|| {
            Some(
                std::env::temp_dir()
                    .join("FancierUsageMonitor")
                    .join("updates"),
            )
        })
