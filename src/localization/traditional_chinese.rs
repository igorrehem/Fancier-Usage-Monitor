use super::Strings;

pub(super) const UPDATE_VIA_WINGET_LABEL: &str = "透過 WinGet 更新";

pub(super) const STRINGS: Strings = Strings {
    window_title: "Claude Code 使用量監控",
    refresh: "重新整理",
    update_frequency: "更新頻率",
    one_minute: "1 分鐘",
    five_minutes: "5 分鐘",
    fifteen_minutes: "15 分鐘",
    one_hour: "1 小時",
    models: "模型",
    claude_code_model: "Claude Code",
    codex_model: "Codex",
    antigravity_model: "Antigravity",
    settings: "設定",
    open_settings: "設定…",
    start_with_windows: "開機時啟動",
    reset_position: "重置位置",
    language: "語言",
    system_default: "系統預設",
    check_for_updates: "檢查更新",
    checking_for_updates: "正在檢查更新...",
    updates: "更新",
    update_in_progress: "已有更新檢查正在進行中。",
    up_to_date: "您已使用最新版本。",
    up_to_date_short: "已是最新",
    update_failed: "無法自動更新",
    applying_update: "正在套用更新...",
    update_to: "更新至",
    update_available: "有可用更新",
    update_prompt_now: "版本 {version} 已可用。是否立即更新？",
    exit: "結束",
    show_widget: "顯示小工具",
    session_window: "5h",
    weekly_window: "7d",
    now: "現在",
    day_suffix: "天",
    hour_suffix: "時",
    minute_suffix: "分",
    token_expired_title: "驗證錯誤",
    token_expired_body: "請在終端機中執行 'claude'，然後使用 '/login' 並依照提示操作。完成後，請重新整理或重新啟動此應用程式。",
    codex_token_expired_title: "Codex 驗證錯誤",
    codex_token_expired_body: "請在終端機中執行 'codex'，並依照登入提示操作。完成後，請重新整理或重新啟動此應用程式。",
    antigravity_token_expired_title: "Antigravity 驗證錯誤",
    antigravity_token_expired_body: "請開啟 Antigravity 並重新登入。完成後，請重新整理或重新啟動此應用程式。",
    codex_window_title: "Codex 使用量監控",
    antigravity_window_title: "Antigravity 使用量監控",
    second_suffix: "秒",
    section_appearance: "外觀",
    section_font: "字型",
    section_size: "大小",
    section_animations: "動畫",
    section_update: "更新",
    section_presets: "預設集",
    color_calm: "平靜",
    color_attention: "注意",
    color_critical: "危急",
    color_background: "背景",
    color_text: "文字",
    color_divider: "分隔線",
    frequency_custom: "自訂",
    preset_default: "預設",
    preset_glass: "玻璃",
    preset_neon: "霓虹",
    preset_minimal: "簡約",
    weight_regular: "常規",
    weight_semibold: "半粗體",
    weight_bold: "粗體",
    geometry_height: "高度",
    geometry_corner_radius: "圓角半徑",
    geometry_bar_thickness: "長條粗細",
    geometry_label_width: "標籤寬度",
    geometry_text_width: "文字寬度",
    geometry_spacing: "間距",
    field_opacity: "不透明度",
    field_family: "字型家族",
    field_size: "大小",
    field_weight: "粗細",
    field_on: "開啟",
    field_speed: "速度",
    field_intensity: "強度",
    field_threshold: "閾值",
    field_duration: "持續時間",
    field_reduce_motion: "減少動態效果",
    field_frequency: "頻率",
    field_custom_minutes: "自訂（分鐘）",
    group_fill: "填充",
    group_shimmer: "微光",
    group_alert_glow: "警示光暈",
    group_fade_slide: "淡入淡出 / 滑動",
    button_reset: "重設",
    button_cancel: "取消",
    button_save: "儲存",
    presets_intro: "按一下預設集，即可套用至下方的外觀與動畫。儲存以保留變更，取消以放棄變更。",
    rgba_custom: "自訂…",
    preset_category_builtin: "內建",
    preset_category_editors: "程式碼編輯器",
    preset_category_apps: "應用程式",
};
