use super::Strings;

pub(super) const UPDATE_VIA_WINGET_LABEL: &str = "Atualizar via WinGet";

pub(super) const STRINGS: Strings = Strings {
    window_title: "Monitor de Uso do Claude Code",
    refresh: "Atualizar",
    update_frequency: "Frequência de Atualização",
    one_minute: "1 Minuto",
    five_minutes: "5 Minutos",
    fifteen_minutes: "15 Minutos",
    one_hour: "1 Hora",
    models: "Modelos",
    claude_code_model: "Claude Code",
    codex_model: "Codex",
    antigravity_model: "Antigravity",
    settings: "Configurações",
    open_settings: "Configurações…",
    start_with_windows: "Iniciar com o Windows",
    reset_position: "Redefinir Posição",
    language: "Idioma",
    system_default: "Padrão do Sistema",
    check_for_updates: "Busca atualizações",
    checking_for_updates: "Buscando por atualizações...",
    updates: "Atualizações",
    update_in_progress: "Uma verificação de atualização já está em andamento.",
    up_to_date: "Você já tem a versão mais recente.",
    up_to_date_short: "Atualizado",
    update_failed: "Não foi possível atualizar automaticamente",
    applying_update: "Aplicando atualização...",
    update_to: "Atualizar para",
    update_available: "Atualização disponível",
    update_prompt_now: "Versão {version} está disponível. Deseja atualizar agora?",
    exit: "Sair",
    show_widget: "Exibir Widget",
    session_window: "5h",
    weekly_window: "7d",
    now: "agora",
    day_suffix: "d",
    hour_suffix: "h",
    minute_suffix: "m",
    second_suffix: "s",
    token_expired_title: "Erro de Autenticação do Claude Code",
    token_expired_body: "Execute 'claude' em um terminal, use '/login' e siga as instruções. Depois disso, atualize ou reinicie este aplicativo.",
    codex_token_expired_title: "Erro de Autenticação do Codex",
    codex_token_expired_body: "Execute 'codex' em um terminal e siga as instruções de login. Depois disso, atualize ou reinicie este aplicativo.",
    antigravity_token_expired_title: "Erro de Autenticação do Antigravity",
    antigravity_token_expired_body: "Abra o Antigravity e entre novamente. Depois disso, atualize ou reinicie este aplicativo.",
    codex_window_title: "Monitor de uso do Codex",
    antigravity_window_title: "Monitor de uso do Antigravity",
    section_appearance: "Aparência",
    section_font: "Fonte",
    section_size: "Tamanho",
    section_animations: "Animações",
    section_update: "Atualização",
    section_presets: "Predefinições",
    color_calm: "Calmo",
    color_attention: "Atenção",
    color_critical: "Crítico",
    color_background: "Fundo",
    color_text: "Texto",
    color_divider: "Divisor",
    frequency_custom: "Personalizado",
    preset_default: "Padrão",
    preset_glass: "Vidro",
    preset_neon: "Neon",
    preset_minimal: "Minimalista",
    weight_regular: "Regular",
    weight_semibold: "Semi-negrito",
    weight_bold: "Negrito",
    geometry_height: "Altura",
    geometry_corner_radius: "Raio do Canto",
    geometry_bar_thickness: "Espessura da Barra",
    geometry_label_width: "Largura do Rótulo",
    geometry_text_width: "Largura do Texto",
    geometry_spacing: "Espaçamento",
    field_opacity: "Opacidade",
    field_family: "Família",
    field_size: "Tamanho",
    field_weight: "Peso",
    field_on: "Ativado",
    field_speed: "Velocidade",
    field_intensity: "Intensidade",
    field_threshold: "Limite",
    field_duration: "Duração",
    field_reduce_motion: "Reduzir Movimento",
    field_frequency: "Frequência",
    field_custom_minutes: "Personalizado (min)",
    group_fill: "Preenchimento",
    group_shimmer: "Brilho",
    group_alert_glow: "Brilho de Alerta",
    group_fade_slide: "Esmaecer / Deslizar",
    button_reset: "Redefinir",
    button_cancel: "Cancelar",
    button_save: "Salvar",
    presets_intro: "Clique numa predefinição para aplicá-la à Aparência e às Animações abaixo. Salve para manter, Cancele para descartar.",
    rgba_custom: "Personalizado…",
    preset_category_builtin: "Integrado",
    preset_category_editors: "Editores de código",
    preset_category_apps: "Aplicativos",
};
